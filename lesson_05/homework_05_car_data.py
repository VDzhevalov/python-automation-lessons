...
searched_cars = {
    car: car_info for car, car_info in car_data.items()
    if (car_info[1] >= search_criteria[0])
       and (car_info[2] >= search_criteria[1])
       and (car_info[4] <= search_criteria[2])
}
print(f"year>= {search_criteria[0]}, engine_volume >= {search_criteria[1]}, price<= {search_criteria[2]}")
print(searched_cars)

searched_cars = sorted(searched_cars.items(), key=lambda x: x[1][4])
print("All cars \n", searched_cars)
